...
if [ "$WOOF_TARGETARCH" = "x86_64" ]; then
	...
	if [ "$FRUGALIFY" = "yes" ]; then
		if [ -e build/ucode.cpio ]; then
			...
			echo "-f \EFI\BOOT\vmlinuz initrd=\EFI\BOOT\ucode.cpio root=PARTLABEL=${DISTRO_FILE_PREFIX}_uefi_root init=/frugalify rootfstype=ext4 rootwait rw" > /mnt/uefiimagep1/EFI/BOOT/efilinux.cfg
		else
			echo "-f \EFI\BOOT\vmlinuz root=PARTLABEL=${DISTRO_FILE_PREFIX}_uefi_root init=/frugalify rootfstype=ext4 rootwait rw" > /mnt/uefiimagep1/EFI/BOOT/efilinux.cfg
		fi
	else
		if [ -e build/ucode.cpio ]; then
			install -m 644 build/ucode.cpio /mnt/uefiimagep1/EFI/BOOT/ucode.cpio
