...
DISTRO_NAME in PUPMODE 5 behaves like a classic "live CD" distribution. Changes to the layered file system at / reside in RAM and lost on shutdown.
...
DISTRO_NAME in PUPMODE 12 behaves like a "normal" distribution. Changes are saved directly to a save folder or a save file, which is used as the upper, writable layer of the layered file system at /.
...
DISTRO_NAME in PUPMODE 13 behaves like an "immutable" distribution where changes are saved only on user request. Changes to the layered file system at / reside in RAM but can be copied to a save folder or file using `save2flash` and during shutdown.
...
PUPMODE 13 can increase the lifespan of flash drives by reducing the number of writing operations, reduce read times of modified files and reduce the risk of data loss when using an unreliable disk.
...
DISTRO_NAME does **not** support other PUPMODEs that are usually found on Puppy Linux and derivatives.
