...
Upstream woof-CE (`puppylinux-woof-CE/woof-CE`) is a general-purpose **meta** build system. Its source is split into `woof-code` (the engine), `woof-arch` (per-architecture prebuilt binaries), and `woof-distro` (per arch/distro/version config). A `merge2out` step merges a chosen combination into a fresh `woof-out_*` directory, where four scripts run: `0setup`, `1download`, `2createpackages`, `3builddistro`. It supports several compat bases (Slackware, Ubuntu, Debian, Devuan, Void) across multiple architectures, draws from compat-distro repos and Puppy **PET** repos plus the `woof-arch` binaries, builds **squashfs** layers in an **aufs**/overlayfs union, ships a traditional X11 desktop (JWM), and runs on a Puppy host.

This fork is a **flattened, single-target** derivative: one already-merged tree dedicated to building VanillaDpup on a Debian/Devuan base, with the scripts and config at the repository root. There is no `merge2out` and no `woof-code`/`woof-arch`/`woof-distro` split. The pipeline is reshaped: `0setup`/`2createpackages` are gone, the package step folds into `1download`, a dedicated `2buildkernel` is added, and it builds inside a plain Debian container/chroot. The most consequential change is how the root filesystem is built: this fork starts from a real `debootstrap` Debian base and overlays `rootfs-skeleton` on top **conflict-free** (the skeleton ships no `/etc/passwd`, `/etc/shadow`, etc.), whereas upstream builds `/` *from* the skeleton — which carries its own base config in traditional Puppy style — and then layers a Debian base on top via `bdrv.sh` for compatibility.

| Area | Upstream woof-CE | This fork |
|------|------------------|-----------|
| Structure | Meta-system: `merge2out` merges `woof-code` + `woof-arch` + `woof-distro` into `woof-out_*` | Pre-merged single tree at the repo root; no `merge2out` |
| Build stages | `0setup`, `1download`, `2createpackages`, `3builddistro` | `1download`, `2buildkernel`, `3builddistro` |
| Build host | A running Puppy with devx (or `run_woof`) | A plain Debian container/chroot |
...
| Bootloaders & boot-sector provenance | BIOS: **isolinux** in the ISO, plus a prebuilt **`wee.mbr`** blob bundled in the skeleton (`/usr/share/boot-dialog/wee.mbr`, formerly `/usr/share/g4dos/`) written by the on-device installers; grub4dos (`grldr`) is offered in the boot menu. UEFI: **GRUB2** from a prebuilt **`grub2-efi.tar.xz`** bundled under `/usr/local/frugalpup/` (per `mk_iso.sh`, "thanks to jamesbond") | BIOS: **syslinux**, MBR from the Debian `syslinux` package's `mbr.bin`. UEFI: **efilinux** built from source as a petbuild (gnu-efi). No GRUB2, no grub4dos |
| Compat bases | Slackware, Ubuntu, Debian, Devuan, Void; multiple arches | Debian/Devuan only; primarily x86_64 |
| Root filesystem (`/`) construction | Built **from `rootfs-skeleton`** in traditional Puppy style — the skeleton ships the base config itself (`/etc/passwd`, `/etc/shadow`, `/etc/group`, `/etc/fstab`, `/etc/inittab`, sysvinit `rcN.d`, with per-compat-distro variants like `passwd.debian`); `bdrv.sh` then layers a Debian/Devuan base on top for compatibility | Built **from `debootstrap`** — a real Debian/Devuan base owns `/etc/passwd`, `/etc/shadow`, etc.; `rootfs-skeleton` is overlaid **conflict-free**, adding only Puppy-specific files (`rc.d`, `init.d`, `profile.d`, `skel`, `hostname`, `eventmanager`) and keeping any file overrides in `/usr/local` |
...
| Layer format | squashfs | erofs (optionally mounted directly to skip loop devices) |
...
| initrd build | `build.sh` downloads a prebuilt static-binary tarball from the `initrd_progs` releases; gzip cpio | `build.sh` assembles the initrd from the freshly built rootfs's own binaries (busybox + fsck/mount tools + libs via `ldd`); zstd cpio |
| PID 1 | Busybox init chain (auto-login via `inittab`) | Custom minimal C `init` (PAM login as `spot`) |
...
| SFS RAM caching (`pfix=copy`/`ram`) | SFSs are **copied into a tmpfs ramdisk** in the initrd, **synchronously** (blocks boot); that RAM is unreclaimable — the tmpfs file stays mounted as a live layer for the session | `sfslock` **locks the SFSs' pages in the page cache** (no copy) from `rc.sysinit`, **in the background** (non-blocking), bounded to ½ RAM; the pages are reclaimable automatically under memory pressure (PSI/OOM) |
| Storage / TRIM awareness | None | Uses `discard_max_bytes` to classify the device: auto-enables SFS page-cache caching only on non-TRIM (slow flash) media, and runs a weekly `fstrim` service (`/etc/init.d/trim`) on TRIM-capable save devices |
| Save filesystem & encryption | Save file ext2/3/4 (user-chosen), non-sparse, `mke2fs -E nodiscard`; whole-volume **LUKS** (`cryptsetup`) on the save file | Save file ext4 (`-O ^has_journal,encrypt`), sparse; per-directory **fscrypt** via `pfscrypt` (whole-save or home-only), for files and folders |
...
| Default app associations | `pmime` + `/etc/pmime.conf`, a shipped `mimeapps.list`, `default*` wrapper scripts, and the `puppyapps` chooser GUI | Standard freedesktop `mimeapps.list`, built via `xdg-mime default` (as `spot`) and propagated to root and `/etc/skel` |
...
| Firewall (`firewall_ng`) | iptables/ip6tables; ~1500-line "block bad packets" script with router/server-style egress filtering; not enabled by default | nftables; ~500-line default-drop-input allowlist, endpoint-focused; enabled by default; blocks mDNS/SSDP/NAT-PMP |
| MAC address | Manual MAC spoofing via `simple_network_setup` | Randomized automatically on a NIC's first bring-up by default (`mac_changer` + udev rule) |
| Kernel sysctl hardening | None shipped (only VM/IO tuning forced in `rc.sysinit`) | Ships `/etc/sysctl.d/99-puppy.conf` (`kptr_restrict=2`, `yama.ptrace_scope=2`, `unprivileged_bpf_disabled`, `bpf_jit_harden`, `kexec_load_disabled`, `sysrq=0`, network redirect/`rp_filter` hardening), applied via `sysctl --system` |
...
Upstream's session runs as **root**, reached through a multi-step busybox-init auto-login chain rather than a dedicated init: `/etc/inittab` respawns `plogin` on `tty1`; `plogin` does first-boot setup (home dir, busybox SUID applets, password prompt) and execs `getty -n` with `/bin/autologin`, which runs a passwordless `login -f`; the login shell's profile then starts X via `xwin`. The `plogin`/`autologin` plumbing nominally supports choosing the login user, but the non-root-user feature (the `finn` user and `loginmanager`) was broken and removed upstream (commit 9b229cf), leaving only vestigial code, so the session is effectively root-only. This fork replaces the entire chain with its custom `init`, which opens a PAM login session directly for the unprivileged `spot` user.
