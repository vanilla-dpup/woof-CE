...
| Boot chain & login | **busybox `init`**, reading `/etc/inittab` to run `rc.sysinit` and `/usr/sbin/plogin` on `tty1`. `plogin` does first-boot setup and execs `getty` with a passwordless `/bin/autologin` script. The login shell then starts X.Org via `xwin`. | [Custom minimal `init`](rootfs-petbuilds/init/init.c). It runs [rc.sysinit](rootfs-skeleton/etc/rc.d/rc.sysinit), then opens a native Linux-PAM login session on `/dev/console`. The profile shell launches the Wayland compositor. |
...
| Save file filesystem | `ext2/3/4` save files (user-chosen). | Only `ext4`, with journaling disabled to boost flash performance and lifespan, and dynamic space allocation (sparse file). |
